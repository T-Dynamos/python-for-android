...
public class PythonActivity extends SDLActivity {
    ...
    public void removeLoadingScreenMain() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM) { // API 35+
              ViewCompat.setOnApplyWindowInsetsListener(getContentView(), (v, windowInsets) -> {
        ...
        runOnUiThread(new Runnable() {
            public void run() {
                if (SplashParent != null) {
                    ViewGroup parent = (ViewGroup) SplashParent.getParent();
                    if (parent != null) {
                        parent.removeView(SplashParent);
                    }
                    SplashParent = null;
                }
                mLottieView = null;
